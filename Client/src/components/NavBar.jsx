...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5">
      ...
      <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to="/" className="text-black">Home</Link>
      <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to="/movie" className="text-black">Movie</Link>
      <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to="/" className="text-black">Theatres</Link>
      <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to="/" className="text-black">Releases</Link>
      <Link onClick={() => { scrollTo(0, 0); setIsOpen(false); }} to="/favourite" className="text-black">Favourites</Link>
      </div>
      ...
      <div className="flex items-center gap-8">
        ...
        {
          !user ? (
            <button onClick={openSignIn} className="px-4 py-1 sm:px-7 sm:py-2 bg-red-500/40 hover:bg-red-500/80 transition rounded-full font-medium cursor-pointer text-black">LOGIN</button>
          ) : (
            <UserButton />
          )
        }
